...
SalesMan::SalesMan(const SalesMan& Arthur) {
  Ncities = Arthur.Ncities;
  rnd = Arthur.rnd;
  Path = Arthur.Path;
  Cities = Arthur.Cities;
}
...
void SalesMan::SimulatedAnnealing(SalesMan& Init,
                                  string shape,
                                  unsigned int ntemp,
                                  unsigned int nstep) {
  ...
  for(unsigned int i=1; i<=ntemp; ++i) {
    accepted = 0.;
    attempted = 0.;
    beta = i;
    ...
    loss << beta << "  " << Init.AbsoluteLoss() << endl;
  }
...
void Population::SetPath(path p, unsigned int i) {
  Pop[i].SetPath(p);
}

void Population::SetCities(vector<city> cities) {
	for(unsigned int i=0; i<Npop; ++i){
 		Pop[i].SetCities(cities);
 	}
}
...
vector<path> Population::Crossover() {
  ...
  if(r < cross_prob) {
    ...
    // At this position we cut the SalesMen
...
void Population::ParallelEvolve(int rank,
                                unsigned int gen,
                                unsigned int migr,
                                std::string shape,
                                MPI_Status* stat) {

  ofstream loss("./results/loss_"+shape+"_"+to_string(rank)+".dat");
  ofstream lossave("./results/loss_ave_"+shape+"_"+to_string(rank)+".dat");
  vector<path> offsprings(2);
  vector<path> pop(Npop);
  ...
  for(unsigned int j=0; j<gen; ++j) {
    ...
    if(j % migr == 0){
			...
			for(unsigned int k=0; k<2; ++k) {
				...
				tag2 = 2 * k + 1;
				if(rank == swaps[2*k]) {
					path1 = (*this).GetSalesMan(Npop-1).GetPath();
					MPI_Send(&path1.front(), Ncities, MPI_INT, swaps[2*k+1], tag1, MPI_COMM_WORLD);
					MPI_Recv(&path2.front(), Ncities, MPI_INT, swaps[2*k+1], tag2, MPI_COMM_WORLD, stat);
					(*this).SetPath(path2, Npop-1);
				}

				if(rank == swaps[2*k+1]) {
					path2 = (*this).GetSalesMan(Npop-1).GetPath();
					MPI_Recv(&path1.front(), Ncities, MPI_INT, swaps[2*k], tag1, MPI_COMM_WORLD, stat);
					MPI_Send(&path2.front(), Ncities, MPI_INT, swaps[2*k], tag2, MPI_COMM_WORLD);
					(*this).SetPath(path1, Npop-1);
				}
			}
			(*this).OrderPop();
		}
