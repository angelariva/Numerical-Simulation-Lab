...
public:
	...
	void SetPath(path, unsigned int);						// Set path for specific salesman
	void SetCities(vector<city>); 							// Set cities for the whole population
	void PrintPop();
  void PrintLosses();
  ...
  void Evolve(unsigned int, std::string shape); // Evolve the population.
																								// Evolve using MPI:
	void ParallelEvolve(int,											// Rank of the process;
											unsigned int,							// # of generations;
											unsigned int,							// # of generations after migration;
										  std::string shape,  			// shape of the world;
											MPI_Status*);							// status of parallel comp.
};
